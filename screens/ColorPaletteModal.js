...
import { TouchableOpacity } from 'react-native-gesture-handler'; // buttons in Native
...
const ColorPaletteModal = ({ navigation }) => {
  ...
  }, [name, navigation, selectedColors]); // if name is missing; Alert will trigger no matter what
  // everytime there is a change in name, make this a valid callback
  // if selectedColors is missing, it'll always say 3 colors are required
  // just maybe don't use useCallback?
...
const styles = StyleSheet.create({
  ...
  color: {
    flexDirection: 'row', // woahhhh that's why, defaults to 'column'
